import { cleanAiMoney, cleanAiString, cleanId, cleanText, loadHouseholdMemberAliases, requireAiAccess } from '../_shared/aiSupport.ts';
...
interface AnalyzeBillBody { householdId?: unknown; billText?: unknown }
interface BillDraftLine { description: string; totalCents: number; participantIds: string[]; participantAliases: string[] }
  ...
  return {
    ...
    schema: {
      ...
      properties: {
        ...
        lineItems: { type: 'array', items: { type: 'object', additionalProperties: false, properties: { description: { type: 'string' }, totalCents: { type: 'integer' }, participantAliases: { type: 'array', items: { type: 'string', enum: memberAliases } } }, required: ['description', 'totalCents', 'participantAliases'] } },
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const warnings = Array.isArray(raw.warnings) ? raw.warnings.filter((warning): warning is string => typeof warning === 'string').map((warning) => warning.trim().slice(0, 300)).filter(Boolean).slice(0, 20) : [];
    ...
    const lineItems: BillDraftLine[] = rawLines.slice(0, 100).flatMap((entry, index) => {
      if (!entry || typeof entry !== 'object') { warnings.push(`Line ${index + 1} could not be parsed.`); return []; }
      ...
      const participantAliases = Array.isArray(data.participantAliases) ? [...new Set(data.participantAliases.filter((alias): alias is string => typeof alias === 'string' && aliasMap.has(alias)))] : [];
      if (participantAliases.length === 0) warnings.push(`Review who should share line ${index + 1}.`);
      return [{ description: cleanAiString(data.description, `Bill item ${index + 1}`, 120), totalCents, participantAliases, participantIds: participantAliases.map((alias) => aliasMap.get(alias)).filter((id): id is string => typeof id === 'string') }];
    });
    ...
    if (statedTotalCents > 0 && Math.abs(statedTotalCents - calculatedTotalCents) > 1) warnings.push(`Parsed items calculate to €${(calculatedTotalCents / 100).toFixed(2)}, but the bill states €${(statedTotalCents / 100).toFixed(2)}. Review the draft before saving.`);

    return jsonResponse({
      title: cleanAiString(raw.title, 'Household bill', 120), merchantName: cleanAiString(raw.merchantName, '', 120),
      categoryId: isExpenseCategory(raw.categoryId) ? raw.categoryId : 'other', discountCents, feeCents, statedTotalCents, calculatedTotalCents,
      lineItems, warnings: [...new Set(warnings)].slice(0, 20),
      requiresReview: lineItems.length === 0 || lineItems.some((line) => line.participantIds.length === 0) || warnings.length > 0,
    });
  } catch (error) {
