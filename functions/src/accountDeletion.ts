...
function requireRecentAuth(auth: { uid: string; token?: Record<string, unknown> } | undefined): string {
  if (!auth?.uid) {
...
export const deleteAccount = onCall(
  ...
  async (request) => {
    const uid = requireRecentAuth(request.auth);
    ...
    await db.runTransaction(async (transaction) => {
      ...
      for (const membership of membershipSnapshot.docs) {
        freshMemberships.push(await transaction.get(membership.ref));
      }
